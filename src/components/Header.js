...
export const Header = () => {
  ...
  const handleSubmit = (event) => {
    ...
    return navigate(`/search?q=${encodeURIComponent(query)}&language=uk-UA`);
  }

  return (
    <header>
      <nav className="bg-amber-200 border-b-2 border-gray-200 dark:bg-black dark:border-b-2 dark:border-gray-200 sm:px-4 py-2">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
            <img src={Logo} className="mr-2 h-8 rounded-md" alt="Kinobratva Logo" />
        ...
        <div id="mobile-nav" className="flex md:order-2">
          ...
          <div className={`items-center justify-between ${hidden ? "hidden" : ""} w-full md:flex md:w-auto md:order-1`} id="navbar-search">
            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-amber-200 dark:bg-gray-800 md:dark:bg-black dark:border-gray-700">
              ...
              <li>
                <NavLink to="/movies/popular" className={({isActive}) => isActive ? activeClass : inactiveClass}>Популярні</NavLink>
              </li>
              <li>
                <NavLink to="/movies/top" className={({isActive}) => isActive ? activeClass : inactiveClass}>Найкращі</NavLink>
              </li>
              <li>
                <NavLink to="/movies/upcoming" className={({isActive}) => isActive ? activeClass : inactiveClass}>Майбутні</NavLink>
              </li>
            </ul>
          </div>
        </div>
